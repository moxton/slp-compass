...
import { Target, CheckCircle, Clock, Users, Copy, Plus } from "lucide-react";
  ...
  return (
    <div className="space-y-6">
      ...
      <Card>
        ...
        <CardContent>
          <p className="text-lg text-slate-700 leading-relaxed">{plan.longTermGoal}</p>
          <Button
            variant="outline"
            size="sm"
            className="mt-4"
            onClick={() => copyToClipboard(plan.longTermGoal)}
          >
        ...
        <CardContent className="space-y-6">
          {plan.objectives.map((objective, index) => (
            <div key={objective.id} className="space-y-4">
              <div className="flex items-start gap-3">
                ...
                <div className="flex-1">
                  ...
                  <Button
                    ...
                    size="sm"
                    className="mt-3"
                    onClick={() => copyToClipboard(objective.text)}
                  >
              ...
              {index < plan.objectives.length - 1 && <Separator />}
            </div>
          ))}
        </CardContent>
      </Card>
        ...
        <CardContent className="space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
              ...
              <p className="text-slate-700">{plan.treatmentProtocol.duration}</p>
              <p className="text-slate-700">{plan.treatmentProtocol.frequency}</p>
            </div>
          </div>
          ...
          <div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
              {plan.treatmentProtocol.targets.map((target, index) => (
          ...
          <div>
            ...
            <div className="space-y-2">
              {plan.treatmentProtocol.hierarchy.map((level, index) => (
          ...
          <div>
            ...
            <div className="space-y-2">
              {plan.treatmentProtocol.prompts.map((prompt, index) => (
